...
UINT32 rbmk_state::screen_update_rbmk(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	for (y=0;y<8;y++)
	{
		for (x=0;x<64;x++)
		{
			...
			m_gfxdecode->m_gfx[0]->opaque(bitmap,cliprect,(tile&0xfff)+((m_tilebank&0x10)>>4)*0x1000,tile>>12,0,0,x*8,y*32);
			count++;
		}
	}
	...
	for (y=0;y<32;y++)
	{
		for (x=0;x<64;x++)
		{
			...
			m_gfxdecode->m_gfx[1]->transpen(bitmap,cliprect,(tile&0xfff)+((m_tilebank>>1)&3)*0x1000,tile>>12,0,0,x*8,y*8,0);
			count++;
		}
	}
	return 0;
