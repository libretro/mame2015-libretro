...
UINT32 destroyr_state::screen_update_destroyr(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	for (i = 0; i < 16; i++)
	{
		...
		m_gfxdecode->m_gfx[2]->transpen(bitmap,cliprect, num, 0, flipx, 0, horz, 16 * i, 0);
	}
	...
	for (i = 0; i < 8; i++)
	{
		for (j = 0; j < 32; j++)
		{
			...
			m_gfxdecode->m_gfx[0]->transpen(bitmap,cliprect, num, 0, 0, 0, 8 * j, 8 * i, 0);
		}
	}
	...
	for (i = 0; i < 2; i++)
	{
		...
		m_gfxdecode->m_gfx[1]->transpen(bitmap,cliprect, num, 0, 0, 0, horz, vert, 0);
	}

	/* draw waves */
	for (i = 0; i < 4; i++)
	{
		m_gfxdecode->m_gfx[3]->transpen(bitmap,cliprect, m_wavemod ? 1 : 0, 0, 0, 0, 64 * i, 0x4e, 0);
	}
