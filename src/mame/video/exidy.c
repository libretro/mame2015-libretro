...
void exidy_state::check_collision()
{
	...
	if (sprite_1_enabled())
	{
		...
		m_gfxdecode->m_gfx[0]->transpen(m_motion_object_1_vid,clip,
				(*m_spriteno & 0x0f) + 16 * sprite_set_1, 0,
				0, 0, 0, 0, 0);
	}
	...
	m_gfxdecode->m_gfx[0]->transpen(m_motion_object_2_vid,clip,
	...
	if (sprite_1_enabled())
	{
		sx = org_2_x - org_1_x;
		sy = org_2_y - org_1_y;
		m_gfxdecode->m_gfx[0]->transpen(m_motion_object_2_clip,clip,
				((*m_spriteno >> 4) & 0x0f) + 32 + 16 * sprite_set_2, 0,
				0, 0, sx, sy, 0);
	}
