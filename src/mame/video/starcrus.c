...
int starcrus_state::collision_check_s1s2()
{
	...
	/* Draw ship 1 */
			...
			m_gfxdecode->m_gfx[8+((m_s1_sprite&0x04)>>2)]->opaque(*m_ship1_vid,
			clip,
	...
	/* Draw ship 2 */
			...
			m_gfxdecode->m_gfx[10+((m_s2_sprite&0x04)>>2)]->opaque(*m_ship2_vid,
			clip,
...
int starcrus_state::collision_check_p1p2()
{
	...
	if (m_p1_sprite & 0x08)  /* if p1 is a projectile */
	{
		/* Draw score/projectile 1 */
				...
				m_gfxdecode->m_gfx[(m_p1_sprite&0x0c)>>2]->opaque(*m_proj1_vid,
				clip,
	...
	if (m_p2_sprite & 0x08)  /* if p2 is a projectile */
	{
		/* Draw score/projectile 2 */
				...
				m_gfxdecode->m_gfx[4+((m_p2_sprite&0x0c)>>2)]->opaque(*m_proj2_vid,
				clip,
...
UINT32 starcrus_state::screen_update_starcrus(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	/* Draw ship 1 */
			...
			m_gfxdecode->m_gfx[8+((m_s1_sprite&0x04)>>2)]->transpen(bitmap,
			cliprect,
			(m_s1_sprite&0x03)^0x03,
			0,
			(m_s1_sprite&0x08)>>3, (m_s1_sprite&0x10)>>4,
	...
	/* Draw ship 2 */
			...
			m_gfxdecode->m_gfx[10+((m_s2_sprite&0x04)>>2)]->transpen(bitmap,
			cliprect,
			(m_s2_sprite&0x03)^0x03,
			0,
			(m_s2_sprite&0x08)>>3, (m_s2_sprite&0x10)>>4,
	...
	/* Draw score/projectile 1 */
			...
			m_gfxdecode->m_gfx[(m_p1_sprite&0x0c)>>2]->transpen(bitmap,
			cliprect,
			(m_p1_sprite&0x03)^0x03,
			0,
			0,0,
	...
	/* Draw score/projectile 2 */
			...
			m_gfxdecode->m_gfx[4+((m_p2_sprite&0x0c)>>2)]->transpen(bitmap,
			cliprect,
			(m_p2_sprite&0x03)^0x03,
			0,
			0,0,
