...
*/
		...
		m_gfxdecode->m_gfx[1]->transpen(bitmap,cliprect, tile, 0, 0, 0, xpos, ypos, 255);
		source -= 0x20;
	}
}
...
UINT32 funybubl_state::screen_update_funybubl(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	for (y = 0; y < 32; y++)
	{
		for (x = 0; x< 64; x++)
		{
			...
			m_gfxdecode->m_gfx[0]->transpen(bitmap,cliprect, data & 0x7fff, (data & 0x8000) ? 2 : 1, 0, 0, x*8, y*8, 0);
			offs += 2;
		}
	}
