...
UINT32 cvs_state::screen_update_cvs(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	for (offs = 0; offs < 0x0400; offs++)
	{
		...
		m_gfxdecode->m_gfx[gfxnum]->opaque(m_background_bitmap,m_background_bitmap.cliprect(),
				code, color,
				0, 0,
				x, y);
		...
		m_gfxdecode->m_gfx[gfxnum]->opaque(m_collision_background,m_collision_background.cliprect(),
