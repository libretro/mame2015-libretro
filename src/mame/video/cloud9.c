...
UINT32 cloud9_state::screen_update_cloud9(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	for (offs = 0; offs < 0x20; offs++)
		if (spriteaddr[offs + 0x00] != 0)
		{
			...
			m_gfxdecode->m_gfx[0]->transpen(m_spritebitmap,cliprect, which, color, xflip, yflip, x, y, 0);
			if (x >= 256 - 16)
				m_gfxdecode->m_gfx[0]->transpen(m_spritebitmap,cliprect, which, color, xflip, yflip, x - 256, y, 0);
		}
