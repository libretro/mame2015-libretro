...
UINT32 bking_state::screen_update_bking(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	m_gfxdecode->m_gfx[3]->transpen(bitmap,cliprect,
		m_ball2_pic,
		m_palette_bank,
		0, 0,
		m_xld2, m_yld2, 0);
...
void bking_state::screen_eof_bking(screen_device &screen, bool state)
{
	...
	if (state)
	{
		...
		if (m_pc3259_mask == 6) /* player 1 */
		{
			...
			m_gfxdecode->m_gfx[2]->opaque(m_colmap_ball,rect, m_ball1_pic, 0, 0, 0, 0, 0);
		...
		else if (m_pc3259_mask == 3) /* player 2 */
		{
			...
			m_gfxdecode->m_gfx[3]->opaque(m_colmap_ball,rect, m_ball2_pic, 0, 0, 0, 0, 0);
