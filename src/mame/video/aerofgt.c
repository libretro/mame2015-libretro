...
//      oy += (ysize*zoomy+2)/4;
		...
		for (y = 0; y <= ysize; y++)
		{
			...
			for (x = 0; x <= xsize; x++)
			{
				...
				m_gfxdecode->m_gfx[m_sprite_gfx + chip]->prio_zoom_transpen(bitmap,cliprect,
								code,
								color,
								flipx,flipy,
								sx,sy,
...
void aerofgt_state::pspikesb_draw_sprites( screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect )
{
	...
	for (i = 4; i < m_spriteram3.bytes() / 2; i += 4)
	{
		...
		m_gfxdecode->m_gfx[m_sprite_gfx]->transpen(bitmap,cliprect,
				code,
				color,
				flipx,flipy,
				xpos,ypos,15);
...
void aerofgt_state::aerfboot_draw_sprites( screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect )
{
	...
	for (attr_start = m_spriteram3.bytes() / 2 - 4; attr_start >= last; attr_start -= 4)
	{
		...
		m_gfxdecode->m_gfx[m_sprite_gfx + (code >= 0x1000 ? 0 : 1)]->prio_zoom_transpen(bitmap,cliprect,
				code,
				color,
				flipx,flipy,
				sx,sy,
