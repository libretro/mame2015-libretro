	...
	for ( ; s1 < buffered_spriteram16 + 0x4000/2; s1+=4 )   // more reasonable (and it cures MAME lockup in e.g. funcube3 boot)
	{
		...
		switch (num & 0x0700)
		{
			default:
				...
				shadow_depth = 0;
				gfx = m_gfxdecode->m_gfx[machine().rand()&3];
				break;
...
void seta2_state::video_start()
{
	...
	m_gfxdecode->m_gfx[3]->set_granularity(16);
	m_gfxdecode->m_gfx[4]->set_granularity(16);
	m_gfxdecode->m_gfx[5]->set_granularity(16);
