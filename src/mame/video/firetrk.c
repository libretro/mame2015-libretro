...
void firetrk_state::firetrk_draw_car(bitmap_ind16 &bitmap, const rectangle &cliprect, int which, int flash)
{
	...
	else
	{
		...
		m_gfxdecode->m_gfx[gfx_bank]->transpen(bitmap,cliprect, code, color, flip_x, flip_y, x, y, 0);
}
...
void firetrk_state::superbug_draw_car(bitmap_ind16 &bitmap, const rectangle &cliprect, int flash)
{
	...
	int flip_y = *m_car_rot & 0x08;
		...
		m_gfxdecode->m_gfx[gfx_bank]->transpen(bitmap,cliprect, code, color, flip_x, flip_y, 144, 104, 0);
}
						...
						int x, int count, int height)
{
	...
	for (i = 0; i < count; i++)
			m_gfxdecode->m_gfx[0]->opaque(bitmap,cliprect, alpha_ram[i], 0, 0, 0, x, i * height);
}
