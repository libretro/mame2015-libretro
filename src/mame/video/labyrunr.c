...
UINT32 labyrunr_state::screen_update_labyrunr(screen_device &screen, bitmap_ind16 &bitmap, const rectangle &cliprect)
{
	...
	if (~m_k007121->ctrlram_r(space, 3) & 0x20)
	{
		...
		m_k007121->sprites_draw(bitmap, cliprect, m_gfxdecode->m_gfx[0], m_palette, m_spriteram,(m_k007121->ctrlram_r(space, 6) & 0x30) * 2, 40,0,screen.priority(),(m_k007121->ctrlram_r(space, 3) & 0x40) >> 5);
