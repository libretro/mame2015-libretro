...
void taitosj_state::video_start()
{
	...
	m_gfxdecode->m_gfx[0]->set_source(m_characterram);
	m_gfxdecode->m_gfx[1]->set_source(m_characterram);
	m_gfxdecode->m_gfx[2]->set_source(m_characterram + 0x1800);
	m_gfxdecode->m_gfx[3]->set_source(m_characterram + 0x1800);
...
WRITE8_MEMBER(taitosj_state::taitosj_characterram_w)
{
	if (m_characterram[offset] != data)
	{
		if (offset < 0x1800)
		{
			...
			m_gfxdecode->m_gfx[1]->mark_dirty((offset / 32) & 0x3f);
		}
		else
		{
			m_gfxdecode->m_gfx[2]->mark_dirty((offset / 8) & 0xff);
			m_gfxdecode->m_gfx[3]->mark_dirty((offset / 32) & 0x3f);
		}
...
inline gfx_element * taitosj_state::get_sprite_gfx_element(UINT8 which)
{
	...
	return m_gfxdecode->m_gfx[(m_spriteram[SPRITE_RAM_PAGE_OFFSET + offs + 3] & 0x40) ? 3 : 1];
}
...
void taitosj_state::draw_layers()
{
	...
	for (offs = 0; offs < 0x0400; offs++)
	{
		...
		m_gfxdecode->m_gfx[m_colorbank[0] & 0x08 ? 2 : 0]->transpen(m_layer_bitmap[0],m_layer_bitmap[0].cliprect(),
				m_videoram_1[offs],
				m_colorbank[0] & 0x07,
				GLOBAL_FLIP_X,GLOBAL_FLIP_Y,
				8*sx,8*sy,0);

		m_gfxdecode->m_gfx[m_colorbank[0] & 0x80 ? 2 : 0]->transpen(m_layer_bitmap[1],m_layer_bitmap[1].cliprect(),
				m_videoram_2[offs],
				(m_colorbank[0] >> 4) & 0x07,
				GLOBAL_FLIP_X,GLOBAL_FLIP_Y,
				8*sx,8*sy,0);

		m_gfxdecode->m_gfx[m_colorbank[1] & 0x08 ? 2 : 0]->transpen(m_layer_bitmap[2],m_layer_bitmap[2].cliprect(),
				m_videoram_3[offs],
				m_colorbank[1] & 0x07,
				GLOBAL_FLIP_X,GLOBAL_FLIP_Y,
				8*sx,8*sy,0);
