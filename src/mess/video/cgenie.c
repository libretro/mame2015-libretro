	...
	if(m_crt.vertical_displayed || m_crt.horizontal_displayed)
	{
		...
		for( address = 0; address < size; address++ )
		{
			...
			if( m_graphics )
			{
				/* get graphics code */
				code = videoram[i];
				m_gfxdecode->m_gfx[1]->opaque(bitmap,r, code, 0,
					0, 0, r.min_x, r.min_y);
			}
			else
			{
				...
				m_gfxdecode->m_gfx[0]->opaque(bitmap,r, code, m_colorram[i&0x3ff],
					0, 0, r.min_x, r.min_y);
			}
			...
			/* check if cursor turned off */
				...
				m_gfxdecode->m_gfx[0]->opaque(bitmap,rc, 0x7f, m_colorram[i&0x3ff],
	...
	if(m_crt.vertical_displayed || m_crt.horizontal_displayed)
	{
		...
		for( address = 0; address < size; address++ )
		{
			...
			if( m_graphics )
			{
				/* get graphics code */
				code = videoram[i];
				m_gfxdecode->m_gfx[1]->opaque(m_bitmap,r, code, 1,
					0, 0, r.min_x, r.min_y);
				m_gfxdecode->m_gfx[1]->opaque(m_dlybitmap,r, code, 2,
					0, 0, r.min_x, r.min_y);
			}
			else
			{
				...
				/* translate defined character sets */
				code += m_font_offset[(code >> 6) & 3];
				m_gfxdecode->m_gfx[0]->opaque(m_bitmap,r, code, m_colorram[i&0x3ff] + 16,
					0, 0, r.min_x, r.min_y);
				m_gfxdecode->m_gfx[0]->opaque(m_dlybitmap,r, code, m_colorram[i&0x3ff] + 32,
					0, 0, r.min_x, r.min_y);
			}
			...
			if( i == cursor )
			{
				...
				m_gfxdecode->m_gfx[0]->opaque(m_bitmap,rc, 0x7f, m_colorram[i&0x3ff] + 16,
					0, 0, rc.min_x, rc.min_y);
				m_gfxdecode->m_gfx[0]->opaque(m_dlybitmap,rc, 0x7f, m_colorram[i&0x3ff] + 32,
					0, 0, rc.min_x, rc.min_y);
			}
		}
	}
